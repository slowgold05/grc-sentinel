...
def create_governance_decision(engine: Engine, org_id: UUID, system_id: UUID, actor: str, request: GovernanceDecisionCreate) -> GovernanceDecisionRecord:
    ...
    with engine.begin() as connection:
        ...
        connection.execute(
            ...
            {"org_id": org_id, "engagement_id": engagement_id, "decision_id": row["id"], "system_id": system_id},
        )
        values = dict(row)
        values.pop("org_id")
...
def list_governance_decisions(engine: Engine, org_id: UUID, system_id: UUID) -> list[GovernanceDecisionRecord]:
