...
export function FrameworkImpact() {
  ...
  return <div className="mt-8"><form action={compare} className="grid gap-3 rounded-2xl border border-zinc-800 bg-zinc-950 p-6 sm:grid-cols-2"><select required name="old" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Previous version</option>{frameworks.map((item) => <option key={item.id} value={item.id}>{item.name} {item.version}</option>)}</select><select required name="new" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Current version</option>{frameworks.map((item) => <option key={item.id} value={item.id}>{item.name} {item.version}</option>)}</select><button className="rounded-lg bg-teal-400 px-4 py-2 font-semibold text-slate-950 hover:bg-teal-300 sm:col-span-2">Compare and find affected policies</button></form>{error && <p role="alert" className="mt-4 text-rose-300">{error}</p>}{impact && <div className="mt-6 grid gap-4 md:grid-cols-3"><Metric label="Added" values={impact.drift.added} /><Metric label="Changed" values={impact.drift.changed} /><Metric label="Removed" values={impact.drift.removed} /><section className="md:col-span-3"><h2 className="text-xl font-semibold">Affected statements</h2>{impact.statements.map((item) => <article key={item.statement_id} className="mt-3 rounded-xl border border-zinc-800 p-4"><p>{item.text}</p><p className="mt-2 text-xs text-slate-400">{item.control_ids.join(", ")}</p></article>)}{!impact.statements.length && <p className="mt-3 text-slate-400">No tenant policy statements cite the changed controls.</p>}</section></div>}</div>;
}
