...
const reviewOptions = [
  ["glba", "GLBA"], ["pci", "PCI DSS"], ["reg-sp", "Regulation S-P"],
  ["finra", "FINRA"], ["nydfs", "NYDFS"], ["ccpa", "CCPA / CPRA"],
  ["dora", "DORA"], ["mas", "MAS TRM"], ["sox", "SOX 404"],
] as const;
...
export function LiveIntake() {
  ...
  const [openReviews, setOpenReviews] = useState(new Set<string>());
  const [activeReview, setActiveReview] = useState("");
  const [step, setStep] = useState<1 | 2>(1);

  function setReview(id: string, open: boolean) {
  ...
  function suggest(...ids: string[]) {
    setOpenReviews((current) => new Set(Array.from(current).concat(ids)));
    setActiveReview((current) => current || ids[0]);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/engagements`, {
        ...
        body: JSON.stringify({
          company: {
            ...
            public_company: form.get("public_company") === "on",
            ...
            exchange_act_reporting_company: optionalBoolean(form.get("exchange_act_reporting_company")),
            sox_filer_category: form.get("sox_filer_category") || null,
            sox_reporting_period_end: form.get("sox_reporting_period_end") || null,
            sox_management_icfr_assessment_required: optionalBoolean(form.get("sox_management_icfr_assessment_required")),
            sox_auditor_attestation_required: optionalBoolean(form.get("sox_auditor_attestation_required")),
            sox_management_assessment_status: form.get("sox_management_assessment_status") || null,
            sox_attestation_status: form.get("sox_attestation_status") || null,
            sox_scope_confirmed: optionalBoolean(form.get("sox_scope_confirmed")),
            ...
            mas_trm_notice_subject: optionalBoolean(form.get("mas_trm_notice_subject")),
            mas_institution_type: form.get("mas_institution_type") || null,
            mas_trm_notice_number: form.get("mas_trm_notice_number") || null,
            mas_licence_or_approval_confirmed: optionalBoolean(form.get("mas_licence_or_approval_confirmed")),
            mas_legacy_notice_transition_complete: optionalBoolean(form.get("mas_legacy_notice_transition_complete")),
            mas_customer_information_handled: optionalBoolean(form.get("mas_customer_information_handled")),
            mas_critical_system_framework_established: optionalBoolean(form.get("mas_critical_system_framework_established")),
            mas_critical_systems_identified: optionalBoolean(form.get("mas_critical_systems_identified")),
            mas_scope_confirmed: optionalBoolean(form.get("mas_scope_confirmed")),
          },
      ...
      if (!response.ok) {
        ...
        const details = Array.isArray(issue?.detail)
          ? issue.detail.map((item: { loc?: string[]; msg?: string }) => `${item.loc?.slice(2).join(" ") || "Input"}: ${item.msg || "invalid value"}`).join(" · ")
          : typeof issue?.detail === "string" ? issue.detail : "Could not create engagement";
  ...
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6 text-slate-100" aria-labelledby="new-engagement">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-red-500">Sentinel GRC / New assessment</p><h2 id="new-engagement" className="mt-2 text-3xl font-semibold">{step === 1 ? "Tell us about the company" : "Review the regulatory scope"}</h2><p className="mt-2 max-w-3xl text-sm text-slate-400">{step === 1 ? "Start with broad facts. We will only surface the regulatory reviews those facts suggest." : "Complete the selected reviews before creating the assessment. Suggestions are not legal determinations."}</p></div><div className="flex min-w-64 items-center gap-3 text-sm"><span className={`grid size-8 place-items-center rounded-full font-semibold ${step === 1 ? "bg-blue-500 text-white" : "bg-emerald-400 text-black"}`}>{step === 1 ? "1" : "✓"}</span><span className={step === 1 ? "font-semibold text-white" : "text-slate-400"}>Company profile</span><span className="h-px flex-1 bg-zinc-700" /><span className={`grid size-8 place-items-center rounded-full font-semibold ${step === 2 ? "bg-blue-500 text-white" : "bg-zinc-800 text-slate-500"}`}>2</span></div></div>
      <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]">
      <form onSubmit={submit} data-step={step} data-active-review={activeReview} className="scope-form grid gap-4 sm:grid-cols-3">
        ...
        <fieldset className="grid gap-3 rounded-xl border border-zinc-800 p-4 sm:col-span-3"><legend className="px-2 text-sm font-semibold">Scope signals</legend><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="us" type="checkbox" className="accent-blue-500" /> Operates in the US</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="financial_services" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("glba", "nydfs")} /> Provides financial services</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="card_data" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("pci")} /> Handles cardholder data</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="securities" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("reg-sp", "finra")} /> Securities or brokerage</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="california" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("ccpa")} /> California consumer data</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="eu" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("dora")} /> EU financial operations</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="singapore" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("mas")} /> Singapore financial operations</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="public_company" type="checkbox" className="accent-blue-500" onChange={(event) => event.target.checked && suggest("sox")} /> Public company</label>
          <label className="flex items-center gap-3 rounded-lg border border-zinc-700 px-3 py-3 text-sm"><input name="email" type="checkbox" className="accent-blue-500" /> Sends external email</label>
        </div></fieldset>
        <div className="step-one-actions flex justify-end sm:col-span-3"><button type="button" onClick={(event) => event.currentTarget.form?.reportValidity() && setStep(2)} className="rounded-lg bg-teal-400 px-5 py-2.5 font-semibold text-black hover:bg-teal-300">Continue to regulatory review →</button></div>
        <div className="review-picker sm:col-span-3"><button type="button" onClick={() => setStep(1)} className="mb-5 text-sm font-medium text-teal-300 hover:text-teal-200">← Edit company profile</button><h3 className="text-xl font-semibold">Suggested regulatory reviews</h3><p className="mt-1 text-sm text-slate-400">Choose a review below. Only one question panel opens at a time.</p><div className="mt-4 grid gap-2 md:grid-cols-2">{reviewOptions.map(([id, title]) => { const selected = openReviews.has(id); const active = activeReview === id; return <button key={id} type="button" aria-expanded={active} onClick={() => { if (!selected) setReview(id, true); setActiveReview(active ? "" : id); }} className={`flex items-center justify-between rounded-lg border px-4 py-3 text-left text-sm font-medium transition ${active ? "border-blue-400 bg-blue-400/10 text-blue-200" : "border-zinc-700 text-slate-300 hover:border-blue-400"}`}><span>{selected ? "✓ " : "+ "}{title}</span><span className="text-xs text-slate-500">{active ? "Close" : "Review"}</span></button>; })}</div></div>
        ...
        <fieldset className="grid gap-3 rounded-lg border border-zinc-700 p-3 sm:col-span-3"><legend className="px-2 text-sm font-semibold text-red-400">MAS Technology Risk Management Notice scope</legend><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><select name="mas_institution_type" defaultValue="" aria-label="MAS institution category" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Institution category unknown</option><option value="licensed_insurer_or_insurance_agent">Licensed insurer / insurance agent</option><option value="bank">Bank</option><option value="credit_or_charge_card_issuer">Credit / charge card issuer</option><option value="finance_company">Finance company</option><option value="merchant_bank">Merchant bank</option><option value="payment_or_dpt_entity">Payment-system / DPT entity</option><option value="money_broker">Money broker</option><option value="licensed_credit_bureau">Licensed credit bureau</option><option value="registered_insurance_broker">Registered insurance broker</option><option value="capital_markets_financial_institution">Capital-markets financial institution</option><option value="licensed_financial_adviser">Licensed financial adviser</option><option value="licensed_trust_company">Licensed trust company</option></select><select name="mas_trm_notice_number" defaultValue="" aria-label="MAS TRM notice number" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Notice number unknown</option>{["FSM-N03", "FSM-N05", "FSM-N07", "FSM-N09", "FSM-N11", "FSM-N13", "FSM-N15", "FSM-N17", "FSM-N19", "FSM-N21", "FSM-N23", "FSM-N25"].map((notice) => <option key={notice} value={notice}>{notice}</option>)}</select>{[["mas_trm_notice_subject", "Subject to selected notice"], ["mas_licence_or_approval_confirmed", "Licence / approval confirmed"], ["mas_legacy_notice_transition_complete", "Current FSM notice transition confirmed"], ["mas_customer_information_handled", "Handles customer information"], ["mas_critical_system_framework_established", "Critical-system framework established"], ["mas_critical_systems_identified", "Critical systems identified"], ["mas_scope_confirmed", "Scope reviewed and confirmed"]].map(([name, label]) => <select key={name} name={name} defaultValue="" aria-label={label} className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">{label}: unknown</option><option value="yes">{label}: yes</option><option value="no">{label}: no</option></select>)}</div><p className="text-xs text-slate-400">A covered institution may identify no critical systems, but still needs the identification framework described by MAS.</p></fieldset>
        <fieldset className="grid gap-3 rounded-lg border border-zinc-700 p-3 sm:col-span-3"><legend className="px-2 text-sm font-semibold text-red-400">SOX Section 404 / ICFR objective</legend><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><select name="sox_filer_category" defaultValue="" aria-label="SOX filer category" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Filer category unknown</option><option value="large_accelerated_filer">Large accelerated filer</option><option value="accelerated_filer">Accelerated filer</option><option value="non_accelerated_filer">Non-accelerated filer</option><option value="emerging_growth_company">Emerging growth company</option><option value="newly_public_transition">Newly public transition</option><option value="registered_investment_company">Registered investment company</option><option value="asset_backed_issuer">Asset-backed issuer</option><option value="private_company">Private company</option><option value="not_determined">Not determined</option></select><input name="sox_reporting_period_end" type="date" aria-label="SOX reporting period end" className="rounded-lg border border-zinc-700 bg-black px-3 py-2" /><select name="sox_management_assessment_status" defaultValue="" aria-label="Management ICFR assessment status" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Management assessment unknown</option><option value="effective">Effective</option><option value="ineffective">Ineffective</option><option value="not_completed">Not completed</option><option value="not_determined">Not determined</option></select><select name="sox_attestation_status" defaultValue="" aria-label="Auditor ICFR attestation status" className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">Auditor attestation unknown</option><option value="unqualified">Unqualified</option><option value="adverse">Adverse</option><option value="disclaimer">Disclaimer</option><option value="not_required">Not required</option><option value="not_completed">Not completed</option><option value="not_determined">Not determined</option></select>{[["exchange_act_reporting_company", "Exchange Act reporting company"], ["sox_management_icfr_assessment_required", "Management ICFR assessment required"], ["sox_auditor_attestation_required", "Section 404(b) attestation required"], ["sox_scope_confirmed", "Objective scope reviewed"]].map(([name, label]) => <select key={name} name={name} defaultValue="" aria-label={label} className="rounded-lg border border-zinc-700 bg-black px-3 py-2"><option value="">{label}: unknown</option><option value="yes">{label}: yes</option><option value="no">{label}: no</option></select>)}</div><p className="text-xs text-slate-400">SOX is represented as an ICFR reporting and audit objective; IT controls support reliable financial reporting.</p></fieldset>
        ...
        <button type="submit" className="rounded-lg bg-teal-400 px-4 py-2 font-semibold text-slate-950 hover:bg-teal-300 sm:col-span-3">Review and create assessment</button>
      </form>
      <aside className="h-fit rounded-xl border border-zinc-800 bg-black/60 p-5 xl:sticky xl:top-6"><h3 className="text-sm font-semibold uppercase tracking-wide text-slate-300">Assessment progress</h3><dl className="mt-5 space-y-4 text-sm"><div className="flex justify-between gap-3"><dt className="text-slate-400">Current step</dt><dd className="font-semibold">{step} of 2</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-400">Reviews selected</dt><dd className="font-semibold text-blue-300">{openReviews.size} of 9</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-400">Legal decisions</dt><dd className="font-semibold text-amber-300">Pending evaluation</dd></div></dl><div className="mt-5 border-t border-zinc-800 pt-5"><h4 className="text-sm font-semibold">Why this is safe</h4><p className="mt-2 text-sm leading-6 text-slate-400">Scope signals only prioritize questions. They never declare a regulation applicable.</p></div></aside>
      </div>
      {result && <p className="mt-4 text-sm font-medium text-emerald-300">{result}</p>}
      {error && <p role="alert" className="mt-4 text-sm text-rose-300">{error}</p>}
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {engagements.map((engagement) => <article key={engagement.id} className="rounded-xl border border-zinc-800 bg-black/70 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{engagement.company.company_name}</h3><p className="mt-1 text-xs text-slate-500">Created {new Date(engagement.created_at).toLocaleString()}</p></div><span className="rounded-full bg-blue-400/10 px-2 py-1 text-xs text-blue-300">Assessment</span></div><p className="mt-3 text-sm text-slate-400">{engagement.company.domain} · {engagement.regulations.join(", ") || "No matched regulation"}</p>{engagement.assurance_objectives.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{engagement.assurance_objectives.map((objective) => <span key={objective.framework} className="rounded-full bg-violet-400/10 px-2 py-1 text-xs text-violet-300">{objective.framework} · {objective.basis.replaceAll("_", " ")}</span>)}</div>}<div className="mt-4 flex flex-wrap gap-x-4 gap-y-2"><label className="cursor-pointer text-sm font-medium text-teal-300">Attach policy<input type="file" accept=".pdf,.docx" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) upload(engagement.id, file); }} /></label><button onClick={() => inspectPosture(engagement.id)} className="text-sm font-medium text-teal-300 hover:text-teal-200">Run posture check</button><button onClick={() => inspectCoverage(engagement.id)} className="text-sm font-medium text-teal-300 hover:text-teal-200">View coverage</button><button onClick={() => inspectReadiness(engagement.id)} className="text-sm font-medium text-violet-300 hover:text-violet-200">View readiness</button><button onClick={() => createAuditShare(engagement.id)} className="text-sm font-medium text-teal-300 hover:text-teal-200">Create audit link</button><button onClick={() => remove(engagement.id)} className="text-sm font-medium text-rose-300 hover:text-rose-200">Delete</button></div></article>)}
      </div>
