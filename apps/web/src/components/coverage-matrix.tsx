...
export function CoverageMatrix({ rows }: { rows: CoverageRow[] }) {
  ...
  return (
    <section aria-labelledby="coverage-heading">
      ...
      <div className="grid overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60 shadow-2xl shadow-black/20 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.8fr)]">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-800/80">
              {rows.map((row) => (
                <tr key={row.control} className={selected?.control === row.control ? "bg-cyan-400/[0.06]" : "hover:bg-slate-800/40"}>
                  <td className="p-0"><button type="button" onClick={() => setSelected(row)} className="w-full px-5 py-5 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-400"><span className="block font-mono text-sm font-semibold text-cyan-300">{row.control}</span><span className="mt-1 block text-sm text-slate-300">{row.title}</span></button></td>
...
function Metric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return <div className={`rounded-xl border p-5 ${accent ? "border-cyan-400/30 bg-cyan-400/[0.07]" : "border-slate-800 bg-slate-900/60"}`}><p className="text-xs font-medium uppercase tracking-wider text-slate-500">{label}</p><p className={`mt-2 text-3xl font-semibold ${accent ? "text-cyan-300" : "text-slate-100"}`}>{value}</p></div>;
}
