        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-800/80">
              {rows.map((row) => (
                <tr key={row.control} className={selected?.control === row.control ? "bg-blue-500/[0.08]" : "hover:bg-slate-800/40"}>
                  <td className="p-0"><button type="button" onClick={() => setSelected(row)} className="w-full px-5 py-5 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500"><span className="block font-mono text-sm font-semibold text-blue-300">{row.control}</span><span className="mt-1 block text-sm text-slate-300">{row.title}</span></button></td>
...
function Metric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return <div className={`rounded-xl border p-5 ${accent ? "border-blue-500/30 bg-blue-500/[0.07]" : "border-zinc-800 bg-zinc-950"}`}><p className="text-xs font-medium uppercase tracking-wider text-slate-500">{label}</p><p className={`mt-2 text-3xl font-semibold ${accent ? "text-blue-300" : "text-slate-100"}`}>{value}</p></div>;
}
